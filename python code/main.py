...
if __name__ == "__main__":
    ...
    # Kp=29, Kd=1.5, sigma=19, Tmid=1, lowsteps=20, v=0.01, c_41, c_31 (0.2, 0.1)
    # Kp=28, Kd=1.6, sigma=17, Tmid=1, lowsteps=20, v=0.01, c_41, c_31 (0.2, 0.1)
    # Now working: Kp=30, Kd=1.7, sigma=20, Tmid=1, lowsteps=20, v=0.01, c_41, c_31 (0.2, 0.1)
    # Now working long case(circular): Kp=20, Kd=1.4, sigma=20, Tmid=1, lowsteps=20, v=0.01, c_35, c_34 (2.55, 0.65)
    ...
    idList = ["1234"]
    # x0List = np.array([-0.2, 0.5, 0, 0]).reshape(1,-1)
    x0List = np.array([0.2, 1.6, 0, 0]).reshape(1,-1)
    ...
    with open('ltl_sequence.pkl', 'rb') as f:
        motionPlan1 = pickle.load(f)
    print(motionPlan1)
    # motionPlan1 = ["c_11", "c_12"]
    # exit()
    # motionPlan1 = ["c_13", "c_14"]
    # motionPlan1 = ["c_31", "c_32", "c_33", "c_23", "c_22", "c_12"]
    # motionPlan1 = ["c_35", "c_45"]
    # motionPlan1 = ["c_35", "c_34", "c_24", "c_25", "c_26", "c_36", "c_35"]
    ...
    # for t in range(simsteps):
        ...
        for i in range(robots.numRobots):
            ...
            t += 1
            # TODO: Uncomment to use low layer/estimated values as mid layer xc
            # robots.robotList[i].midSystem.xc[0] = robots.robotList[i].lowSystem.x
            # robots.robotList[i].midSystem.xc[1] = robots.robotList[i].lowSystem.y
            robots.robotList[i].midSystem.get_mpc(robots.robotList[i].midControl)
        ...
        break_condition = all(robots.robotList[k].topSystem.terminate for k in range(robots.numRobots))
            # break
    ...
    for i in range(robots.numRobots):
        ...
        xc_low = np.concatenate((np.array([robots.robotList[i].lowSystem.x, robots.robotList[i].lowSystem.y, robots.robotList[i].lowSystem.theta]), np.zeros(2)))
